...
public class AdminController {
	...
	/*
	 * クイズ新規登録画面 機能: csvファイルからクイズを一括登録する
	 */
	...
	public String uploadFile(@RequestParam("file") MultipartFile uploadFile, RedirectAttributes redirectAttributes) {
		try (BufferedReader br = new BufferedReader(
			...
			while ((line = br.readLine()) != null) {
				...
				final QuizDto quizDto = QuizDto.builder().quiz((split[0])).answer1(split[1]).answer2(split[2])
						.answer3(split[3]).build();
	...
	/*
	 * クイズ一覧画面 クイズ編集モーダル 機能: クイズを更新する
	 */
	@RequestMapping(value = "/update", method = RequestMethod.POST)
	public String updateQuiz(@ModelAttribute QuizForm form, Quiz quiz, RedirectAttributes redirectAttributes) {
		try {
			quizMapper.updateQuiz(form);
			// 登録完了後、メッセージフラグをtrueに設定する
			successMsgFlg = true;
			// リダイレクト先にパラメータを渡す
			redirectAttributes.addFlashAttribute("successMsgFlg", successMsgFlg);
